...
DPI = 96
DATASET = "data"
DATASET_LEFT = DATASET + "/left_images/"
DATASET_RIGHT = DATASET + "/right_images/"
...
def process_frame(left, right, name):
    ...
    imgR_gray = cv2.cvtColor(left, cv2.COLOR_BGR2GRAY)
    imgL_gray = cv2.cvtColor(right, cv2.COLOR_BGR2GRAY)
 
    # Applying stereo image rectification on the left image
    left = cv2.remap(imgL_gray,
              Left_Stereo_Map_x,
              Left_Stereo_Map_y,
              cv2.INTER_LANCZOS4,
              cv2.BORDER_CONSTANT,
